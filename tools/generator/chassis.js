...
function runJava(src){
  ...
  function scanTop(s, ops){ // split left-to-right on single-char ops, honoring quotes/parens; skips a leading (unary) operator
    const toks=[], found=[]; let depth=0,inStr=false,cur='';
    for(let i=0;i<s.length;i++){ const c=s[i];
      if(c==='"'&&s[i-1]!=='\\\\') inStr=!inStr;
      if(!inStr){ if(c==='(')depth++; else if(c===')')depth--;
        else if(depth===0 && ops.indexOf(c)>=0 && cur.trim()!==''){ toks.push(cur); found.push(c); cur=''; continue; } }
      cur+=c; }
    toks.push(cur); return {toks, found}; }
  function multiplicative(expr){ // * / % left-to-right (same precedence)
    const {toks, found}=scanTop(expr, ['*','/','%']);
    let acc=evalAtom(toks[0]);
    for(let i=0;i<found.length;i++){ const r=evalAtom(toks[i+1]), o=found[i];
      if(o==='*') acc={type:'num',value:acc.value*r.value};
      else if(o==='/') acc={type:'num',value:Math.trunc(acc.value/r.value)};
      else acc={type:'num',value:acc.value%r.value}; }
    return acc; }
  function additive(expr){ // + - left-to-right ('+' concatenates if either side is a String)
    const {toks, found}=scanTop(expr, ['+','-']);
    let acc=multiplicative(toks[0]);
    for(let i=0;i<found.length;i++){ const r=multiplicative(toks[i+1]);
      if(found[i]==='+') acc=(acc.type==='String'||r.type==='String') ? {type:'String',value:toStr(acc)+toStr(r)} : {type:'num',value:acc.value+r.value};
      else acc={type:'num',value:acc.value-r.value}; }
    return acc; }
  function evalExpr(expr){
    expr=expr.trim();
    const cmp=findTop(expr,['>=','<=','==','!=']) || findTop(expr,['>','<']);
    if(cmp){ const l=additive(expr.slice(0,cmp.i)); const r=additive(expr.slice(cmp.i+cmp.op.length));
